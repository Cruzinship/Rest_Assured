...
import com.cydeo.day06DeserializationContinued.utilities.NewSpartanTestBase;
...
public class P02_SpartansSpecTest extends NewSpartanTestBase {
    ...
    @Test
    public void getAllSpartansWithReqSpec() {
        given().spec(reqSpec)
                .when()
                .get("/spartans")
                .then()
    ...
    public void getSingleSpartan_GET_RBAC(String username, String password, int id, int statusCode) {
        given().spec(dynamicReqSpec(username, password))
                ...
                .get("/spartans/{id}")
                .then()
                .spec(dynamicReqSpec(statusCode));
    ...
    public void getSingleSpartan_DELETE_RBAC(String username, String password, int id, int statusCode) {

        given().spec(dynamicReqSpec(username, password))
                .pathParam("id", id)
                .when().delete("/spartans/{id}")
                .then().spec(dynamicReqSpec(statusCode));
